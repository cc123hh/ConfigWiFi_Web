...
class _FreeD:
    ...
    def __init__(self, pitch: 'int', yaw: 'int', roll: 'int', pos_z: 'int',  pos_y: 'int', pos_x: 'int', zoom: 'int', focus: 'int') -> None:
        ...
        self.focus = focus.to_bytes(3, 'big')
    ...
    def checksum(*args) -> 'bytes':
        ...
        return checksum.to_bytes(1, 'big', False)
...
class ThreeBytes(int):
    ...
    def to_bytes(self, length, byteorder:str, *, signed: bool = ...) -> bytes:
        if byteorder not in ["little", "big"]:
            raise Exception("arg_val must in : little,big")
        return self.field.to_bytes(length, byteorder, signed)
...
class FreeDWrapper:
    ...
    def createFreeD(self) -> '_FreeD':
        ...
        struct.cameraID = self.cameraid.to_bytes(1, 'big', False) 
